...
### Task 1: Add `objectives` field to profile model

**Files:**
- Modify: `backend/services/profile.py:5-14`
- Modify: `backend/routers/profile.py:44-71`
- Modify: `src/api/profile.ts:6-13`
- Test: `backend/tests/test_profile_api.py`

- [ ] **Step 1: Write failing test for objectives in profile service**
...
Run: `python -m pytest backend/tests/test_profile_service.py::test_empty_profile_includes_objectives -v`
Expected: FAIL — `objectives` not in `EMPTY_PROFILE`

- [ ] **Step 3: Add objectives to profile service**

In `backend/services/profile.py`:

```python
VALID_SECTIONS = {"summary", "skills", "experience", "activities", "education", "certifications", "objectives"}
...
- [ ] **Step 5: Write failing test for objectives PATCH endpoint**

Add to `backend/tests/test_profile_api.py`:
...
Run: `python -m pytest backend/tests/test_profile_api.py::test_patch_objectives -v`
Expected: FAIL — `objectives` not in `_SECTION_VALIDATORS`

- [ ] **Step 7: Add objectives to ProfileRequest and validators**

In `backend/routers/profile.py`, add to `ProfileRequest`:
...
- [ ] **Step 9: Update frontend Profile type**

In `src/api/profile.ts`, add `objectives` to the `Profile` interface:
...
- [ ] **Step 10: Run all profile tests to confirm no regressions**

Run: `python -m pytest backend/tests/test_profile_api.py backend/tests/test_profile_service.py -v`
Expected: ALL PASS

- [ ] **Step 11: Commit**

```bash
git add backend/services/profile.py backend/routers/profile.py src/api/profile.ts backend/tests/test_profile_service.py backend/tests/test_profile_api.py
git commit -m "feat: add objectives field to profile model"
```
